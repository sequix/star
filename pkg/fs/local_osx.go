...
func (r *LocalReader) Next() (*File, error) {
	...
	fh := &File{
		FileInfo: FileInfo{
			...
			Uid:      fsi.Uid,
			Gid:      fsi.Gid,
			Mtime:    time.Unix(fsi.Mtimespec.Sec, fsi.Mtimespec.Nsec),
			Atime:    time.Unix(fsi.Atimespec.Sec, fsi.Atimespec.Nsec),
			Ctime:    time.Unix(fsi.Ctimespec.Sec, fsi.Ctimespec.Nsec),
			Mode:     fi.Mode(),
